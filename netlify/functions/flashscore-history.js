// Netlify serverless function — FlashScore History (Finished Results)
// Endpoint: /.netlify/functions/flashscore-history  (GET or POST)
...
exports.handler = async (event) => {
  ...
  // Accept dateOffset from query or body (-1 for yesterday, -2 for 2 days ago)
  let dateOffset = -1;
  try {
  ...
  if (event.queryStringParameters?.dateOffset !== undefined) {
    dateOffset = parseInt(event.queryStringParameters.dateOffset) || -1;
  }
  ...
  try {
    ...
    console.error('flashscore-history error:', err.message);
    return {
      statusCode: 500,
      headers: corsHeaders(),
